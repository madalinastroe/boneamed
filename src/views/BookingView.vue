...
<template>
  <section class="booking-page">
    <div class="booking-top"><RouterLink class="brand compact" to="/"><strong>Bonea Med</strong></RouterLink><span>Programare online</span></div>
    <div class="progress" aria-label="Progres programare"><div v-for="(label, index) in steps" :key="label" :class="{ active: step >= index + 1 }"><span>{{ index + 1 }}</span><small>{{ label }}</small></div></div>
    ...
    <div class="booking-card">
      ...
      <template v-else-if="step === 2">
        <button class="back" @click="step = 1">← Înapoi</button><p class="selected-doctor">{{ state.doctor?.name }}</p><h1>Alege data și ora</h1><p class="muted">Sunt afișate doar datele disponibile în următoarele 8 săptămâni.</p>
